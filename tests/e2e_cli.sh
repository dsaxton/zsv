...
run_case \
  "greatest+table one header" \
  $'name  | score\n------+------\nCara  | 10   \nAlice | 9    \nBob   | 8    \nDan   | 7    \n' \
  --greatest score -t -s name,score -n 4

run_case \
  "greatest+csv one header" \
  $'name,score\nCara,10\nAlice,9\nBob,8\nDan,7\n' \
  --greatest score -s name,score -n 4

run_case \
  "greatest+table+no-header" \
  $'Cara  | 10   \nAlice | 9    \nBob   | 8    \nDan   | 7    \n' \
  --greatest score -t --no-header -s name,score -n 4

run_case \
  "greatest+csv+no-header" \
  $'Cara,10\nAlice,9\nBob,8\nDan,7\n' \
  --greatest score --no-header -s name,score -n 4

run_case \
  "greatest+filter+select" \
  $'name,score\nCara,10\nAlice,9\n' \
  --greatest score -f dept=Eng -s name,score -n 2

run_case \
  "greatest+filter+table" \
  $'name  | score\n------+------\nCara  | 10   \nAlice | 9    \n' \
  --greatest score -f dept=Eng -t -s name,score -n 2

run_case \
  "least+csv" \
  $'name,score\nDan,7\nBob,8\n' \
  --least score -s name,score -n 2

run_case \
  "least+table" \
  $'name | score\n-----+------\nDan  | 7    \nBob  | 8    \n' \
  --least score -t -s name,score -n 2

run_case \
  "greatest+head-default when -n has no value" \
  $'name,score\nCara,10\nAlice,9\nBob,8\nDan,7\n' \
  --greatest score -n -s name,score
